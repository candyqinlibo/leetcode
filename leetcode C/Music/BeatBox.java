...
public class BeatBox {
	...
	public void buildTrackAndStart(){
		...
		for (int i = 0; i < 16; i++) {
			...
			for (int j = 0; j < 16; j++) {
				JCheckBox jcBox = (JCheckBox)checkboxList.get(j+(16*i));
				if (jcBox.isSelected()) {
					trackList[j] = key;
				}else{
					trackList[j] = 0;
	...
	public class MyReadListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				ObjectInputStream isInputStream = new ObjectInputStream(fileInputStream);
				checkbox = (boolean[])isInputStream.readObject();
			} catch (Exception e2) {
				// TODO: handle exception
				e2.printStackTrace();
			}
